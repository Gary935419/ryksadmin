        ...
        <div class="layui-col-md12">
            <div class="layui-card">
                <div class="layui-card-body ">
                    <form class="layui-form layui-col-space5" method="get" action="<?= RUN, '/goods/goods_list_zi' ?>">
                ...
                <button class="layui-btn layui-card-header" style="float: right;margin-top: -40px;margin-right: 20px;"
                        onclick="xadmin.open('添加','<?= RUN . '/goods/goods_add_zi' ?>',1000,600)"><i
                    ...
                    <table class="layui-table layui-form">
                        ...
                        <?php if (isset($list) && !empty($list)) { ?>
                            <?php foreach ($list as $num => $once): ?>
                                <tr style="<?php echo $num%2 != 0 ? 'background-color: #f8f8f8': '' ?>" id="p<?= $once['gid'] ?>" sid="<?= $once['gid'] ?>">
                                    <td><?= $num + 1 ?></td>
                                    <td><?= $once['gname'] ?></td>
									<td><?= $once['gsort'] ?></td>
                                    ...
                                    <td class="td-manage">
                                        <button class="layui-btn layui-btn-normal"
                                                onclick="xadmin.open('编辑','<?= RUN . '/goods/goods_edit_zi?gid=' ?>'+'<?= $once['gid'] ?>',1000,600)">
