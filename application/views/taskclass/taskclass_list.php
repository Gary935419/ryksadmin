        ...
        <div class="layui-col-md12">
            <div class="layui-card">
                ...
                <button class="layui-btn layui-card-header" style="float: right;margin-top: -40px;margin-right: 20px;"
                        onclick="xadmin.open('添加','<?= RUN . '/taskclass/taskclass_add' ?>',1000,600)"><i
                    ...
                    <table class="layui-table layui-form">
                        ...
                        <?php if (isset($list) && !empty($list)) { ?>
                            <?php foreach ($list as $num => $once): ?>
                                <tr id="p<?= $once['tid'] ?>" sid="<?= $once['tid'] ?>">
                                    <td><?= $num + 1 ?></td>
                                    <td><?= $once['tname'] ?></td>
                                    <td><?= $once['tsort'] ?></td>
                                    <td><img src="<?= $once['timg'] ?>" style="width: 50px;height: 50px;"></td>
                                    <td class="td-manage">
                                        <button class="layui-btn layui-btn-normal"
                                                onclick="xadmin.open('编辑','<?= RUN . '/taskclass/taskclass_edit?tid=' ?>'+<?= $once['tid'] ?>,900,500)">
                                            <i class="layui-icon">&#xe642;</i>编辑
                                        </button>
                                        <button class="layui-btn layui-btn-danger"
                                                onclick="taskclass_delete('<?= $once['tid'] ?>')"><i class="layui-icon">&#xe640;</i>删除
                                        </button>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
